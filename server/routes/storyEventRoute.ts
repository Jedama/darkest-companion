...
router.post('/estates/:estateName/events/story', async (req: Request, res: Response) => {
  try {
    ...
    const { event, chosenCharacterIds, locations, npcIds, enemyIds, bystanders, keywords } = req.body;
    ...
    const prompt = await compileStoryPrompt(estate, event, chosenCharacterIds, locations, npcIds, enemyIds, bystanders, keywords);
